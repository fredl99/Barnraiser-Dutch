...
if (isset($_POST['submit_delete_avatar'])) {
	...
	elseif (!is_uploaded_file($_FILES['frm_file']['tmp_name'])) {
		$error_txt = _("The picture you are trying to upload is too big. Please upload a picture less than {size} in size.");
		$error_txt = str_replace("{size}", ini_get('post_max_size'), $error_txt);
